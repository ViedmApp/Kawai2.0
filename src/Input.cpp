...
void Input::initialiceInput(){
  ...
  if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS and !firstPlayer->isSlowed)
  ...
  if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS)
    firstPlayer -> reverse();
  if (glfwGetKey(window, GLFW_KEY_F) == GLFW_PRESS and !bala1 and firstPlayer -> firerate <=0.0f){
     firstPlayer -> shootBullet();
     bala1 = true;
  }
  else if (glfwGetKey(window, GLFW_KEY_F) == GLFW_RELEASE and bala1){
    bala1=false;
  }
  if (glfwGetKey(window, GLFW_KEY_G) == GLFW_PRESS and !trampa1 and mapa -> trampa_P1 -> cdCount <= 0.0f)
  {
    mapa -> crearTrampa(firstPlayer->getXTrap(),firstPlayer->getYTrap(),firstPlayer->getZTrap(),1);
    trampa1 = true;
  }
  else if(glfwGetKey(window, GLFW_KEY_G) == GLFW_RELEASE and trampa1)
  {
    trampa1 = false;
  }


  if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS and !secondPlayer->isSlowed)
  ...
  if (glfwGetKey(window, GLFW_KEY_K) == GLFW_PRESS and !trampa2 and mapa -> trampa_P2 -> cdCount <= 0.0f)
  {
    mapa -> crearTrampa(secondPlayer->getXTrap(),secondPlayer->getYTrap(),secondPlayer->getZTrap(),2);
    trampa2 = true;
  }
  else if(glfwGetKey(window, GLFW_KEY_K) == GLFW_RELEASE and trampa2)
  {
    trampa2 = false;
  }
  if (glfwGetKey(window, GLFW_KEY_J) == GLFW_PRESS and !bala2 and secondPlayer -> firerate <=0.0f){
     secondPlayer -> shootBullet();
     bala2 = true;
  }
  else if (glfwGetKey(window, GLFW_KEY_J) == GLFW_RELEASE and bala2){
    bala2=false;
  }
  if (glfwGetKey(window, GLFW_KEY_I) == GLFW_PRESS)
  {
    btVector3 position = secondPlayer->getPosition();
    secondPlayer -> setPosition(secondPlayer->getX(),secondPlayer->getY(),secondPlayer->getZ());
    secondPlayer->getRigidBody()->setLinearVelocity(btVector3(0,0,0));
    secondPlayer->getRigidBody()->setAngularVelocity(btVector3(0,0,0));
    secondPlayer->updatePhysics();
    secondPlayer->updateTurn();
  }
